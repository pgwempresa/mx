const { envFirst, setCors, sendJson, readBody, kvGetJson, kvSetJson } = require('./mexico-lib');
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET' && !req.url.includes('?')) {
    return sendJson(res, 200, { ok: true, route: 'mexico-xpag-status', mode: getXpagConfig().mode });
  }
  ...
  try {
    ...
    if (!response.ok || body.ok === false) {
      return sendJson(res, response.status || 502, {
        error: 'XPag rechazo la consulta SPEI.',
