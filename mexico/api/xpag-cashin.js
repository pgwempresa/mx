const { envFirst, setCors, sendJson, readBody } = require('./lib');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!Number.isFinite(amount) || amount <= 0) {
      return sendJson(res, 422, { error: 'Monto MXN invalido para XPag.', missing_fields: ['amount'] });
    }
    if (name.length < 3) {
      return sendJson(res, 422, { error: 'Nombre incompleto para XPag.', missing_fields: ['name'] });
    }
    if (document.length < 10) {
      return sendJson(res, 422, { error: 'Documento mexicano incompleto para XPag.', missing_fields: ['document'] });
    }
    ...
    const requestPayload = {
      ...
      name,
      document,
      description: String(input.paymentDescription || input.description || 'Meta Mexico SPEI').slice(0, 80),
      external_id: buildExternalId(input),
      payer: { name, document }
    };
    if (origin && !String(origin).includes('127.0.0.1') && !String(origin).includes('localhost')) {
      requestPayload.webhook_url = `${proto}://${origin}/mexico/api/xpag-webhook.php`;
    }
    ...
    const response = await fetch(config.baseUrl + '/cashin', {
      ...
      headers: {
        ...
        'User-Agent': 'mexico-spei-funnel/1.0'
      },
